...
# search_comp = 'MSFT'
# cik = companyCIK.loc[companyCIK['ticker'] == search_comp, 'cik_str'].iloc[0]
...
cur_liabilities_plot = px.line(cur_liabilities_df_filtered,
                      title=f"APPL Current Liabilities",
                      x="end",
                      y="val")
...
equity_plot = px.line(equity_df_filtered,
                      title=f"APPL Stockholder Equity",
                      x="end",
                      y="val")
equity_plot.show()
...
print(desc_merged_asst_lib_eq_net_df[['assets', 'liabilities']])
...
merged_plot2 = px.line(merged_asst_lib_eq_df,
                      ...
                      title="APPL Total Liabilities & Shareholders' Equity")
...
merged_plot4 = px.line(merged_cur_asst_lib_df,
                      ...
                      title="APPL Current Assets & Current Liabilities")
...
merged_cur_asst_lib_df['current_ratio'] = merged_df['current_assets']/ merged_df['current_liabilities']
# print(merged_asst_lib_df)

current_ratio_plot = px.line(merged_df,
                      x='end_x', y='current_ratio',
                      title="APPL Current Ratio")
current_ratio_plot.update_yaxes(range=[0, max(merged_df['current_ratio'])])
...
roe_ratio_plot = px.line(merged_asst_lib_eq_net_df,
                      x='end', y='roe_ratio',
                      title="APPL Return on Equity Ratio",
                                    labels={
